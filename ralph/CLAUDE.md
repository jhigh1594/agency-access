...
You are an autonomous coding agent working on the Agency Access Platform - an OAuth aggregation platform for marketing agencies.

## Your Task

1. Read the PRD at `ralph/prd.json` (in the ralph directory)
2. Read the progress log at `ralph/progress.txt` (check Codebase Patterns section first)
3. Check you're on the correct branch from PRD `branchName`. If not, check it out or create from main.
4. Pick the **highest priority** user story where `passes: false`
5. Implement that single user story following TDD principles (write tests first, then implementation)
...
- NEVER store OAuth tokens directly in PostgreSQL - always use Infisical
- Store only `secretId` (secret reference) in database
- Log all token access in `AuditLog` table

**Development Rules:**
- Follow Test-Driven Development (TDD) - write tests BEFORE implementation
- Follow patterns in `CLAUDE.md` (project root) exactly
- Use shared types from `packages/shared/src/types.ts`
...
- Example: Never store OAuth tokens in database - always use Infisical
```
...
- Follow TDD - tests must be written before implementation
...
- Never store OAuth tokens in database - use Infisical only
