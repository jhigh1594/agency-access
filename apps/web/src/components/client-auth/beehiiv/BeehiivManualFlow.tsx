/**
 * BeehiivManualFlow Component
 *
 * Main container for the Beehiiv manual invitation flow.
 * Manages state, API communication, and orchestrates child components.
 *
 * Flow:
 * 1. Client views 6-step instructions
 * 2. Client follows steps in Beehiiv (manual process)
 * 3. Client checks "I completed this step"
 * 4. Client clicks "Continue"
 * 5. API creates pending ClientConnection
 * 6. Redirect to Step 2 (Select Publications)
 */
...
import { BeehiivProgressHeader } from './BeehiivProgressHeader';
import { BeehiivInstructionSteps } from './BeehiivInstructionSteps';

interface BeehiivManualFlowProps {
  token: string;
  agencyEmail: string;
  agencyName: string;
  clientEmail?: string;
...
export function BeehiivManualFlow({
  token,
  agencyEmail,
  agencyName,
  clientEmail,
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/client/${token}/beehiiv/manual-connect`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="h-12 w-12 rounded-lg bg-white border border-slate-200 flex items-center justify-center">
            <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M22 7L12 12V22L22 7Z" fill="#FFA000" />
              <path d="M12 12L2 7L12 22L22 7L12 12Z" fill="#FFB800" />
            </svg>
          </div>

          <div>
            <h1 className="text-2xl font-bold text-slate-900">You're almost done!</h1>
            <p className="text-slate-600 mt-1">Assign your Beehiiv Workspace</p>
          </div>
        </div>
      ...
      <div className="px-6 pt-6">
        <BeehiivProgressHeader currentStep={1} />
      </div>

      {/* Instruction Steps */}
      <div className="px-6 py-6">
        <BeehiivInstructionSteps
