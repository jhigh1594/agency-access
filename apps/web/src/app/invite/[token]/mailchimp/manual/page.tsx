...
export default function MailchimpManualPage() {
  ...
  const parseManualData = useCallback((payload: ClientAccessRequestPayload): ManualPageData => {
    return {
      ...
      agencyEmail: payload.manualInviteTargets?.mailchimp?.agencyEmail || 'your agency contact email',
  ...
  const submitManualConnection = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/client/${token}/mailchimp/manual-connect`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || result.error || !result.data?.connectionId) {
        throw new Error(result.error?.message || 'Failed to create Mailchimp connection');
      }

      router.push(`/invite/${token}?step=2&platform=mailchimp&connectionId=${result.data.connectionId}`);
    } catch (err) {
      setSubmissionError(err instanceof Error ? err.message : 'Failed to create Mailchimp connection');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return [
      {
        ...
        description: 'Use this email when inviting your agency into Mailchimp.',
        content: (
      ...
      },
      {
        ...
        title: 'Open Mailchimp team settings',
        description: 'Sign in to Mailchimp and open the account team or user-management area.',
        content: (
          <div className="space-y-3">
            <a
            ...
            >
              Open Mailchimp Login
              <ExternalLink className="h-4 w-4" />
            </a>
            <p className="text-sm text-muted-foreground">
              From your account, open your user or team settings so you can invite a new user.
            </p>
          </div>
        ),
      ...
      {
        id: 'send-invite',
        title: 'Invite your agency in Mailchimp',
        description: 'Invite a new user, paste the email below, choose the appropriate role, then send.',
        content: (
          <div className="rounded-lg border border-border bg-muted/10 px-4 py-3">
            <ul className="space-y-2 text-sm text-foreground list-disc pl-5">
              <li>Select the option to invite a new user or team member.</li>
              <li>Paste <span className="font-mono">{data.agencyEmail}</span>.</li>
              <li>Choose the role you want your agency to have.</li>
              <li>Send the invitation from Mailchimp.</li>
            </ul>
          </div>
        ),
      ...
      },
      {
        ...
        completionGate: {
          label: `I invited ${data.agencyEmail} to my Mailchimp account`,
    ...
    return (
      <InviteLoadStateCard
        ...
        message={
          error ||
          (phase === 'timeout'
            ? 'Mailchimp setup took too long to load. Retry or contact support.'
  ...
  return (
    <InviteFlowShell
      ...
      description="Connect Mailchimp by completing each checklist step."
      header={
        <ManualInviteHeader
          ...
          securityNote="Use only Mailchimp-native invite screens. Never share credentials."
          backAction={
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push(buildClientInviteConnectViewUrl(token, 'mailchimp') as any)}
      ...
      rail={
        <InviteStickyRail
          objective="Complete Mailchimp invite setup and return to your authorization request."
          securityNote="Use only Mailchimp-native invite screens. Never share credentials."
          identities={[{ label: 'Mailchimp invite email', value: data.agencyEmail }]}
