...
export default function BeehiivManualPage() {
  ...
  const submitManualConnection = async () => {
    ...
    try {
      const response = await fetch(resolveApiUrl(`/api/client/${token}/beehiiv/manual-connect`), {
        method: 'POST',
        headers: {
        ...
        <ManualInviteHeader
          ...
          backAction={
            <Button
              variant="ghost"
              size="sm"
              onClick={() => router.push(buildClientInviteConnectViewUrl(token, 'beehiiv') as any)}
