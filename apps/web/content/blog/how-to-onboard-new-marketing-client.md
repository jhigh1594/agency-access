---
id: how-to-onboard-new-marketing-client
title: How to Onboard a New Marketing Client (The Real 2026 Guide)
excerpt: >-
  Every agency onboarding guide says "Day 1: kickoff call. Day 3: first
  campaign." They skip the part where your account manager spends three days
  chasing Meta Business Manager access. Here's how to fix the broken agency
  onboarding timeline — and why the access bottleneck is costing you clients.
category: onboarding
stage: awareness
...
tags:
  ...
  - platform access
  - agency growth
metaTitle: 'How to Onboard a New Marketing Client (The Real 2026 Guide)'
metaDescription: >-
  Every agency onboarding guide skips the platform access bottleneck.
  Here's the real timeline for onboarding a new marketing client — from
  contract signed to first campaign live, including the step most guides
  ignore.
relatedPosts:
...
**Day 3**: Second follow-up. You finally get Google Ads access — but the client sent the invitation to a different account manager who's on PTO. LinkedIn access expired because nobody accepted it within 48 hours.
...
Seventy-four percent of customers will switch to a competitor if onboarding feels too complicated. And in a 2024 survey of 850+ onboarding professionals, 62% said their single biggest time sink was chasing customers for access and information — not strategy, not creative, not reporting. Just chasing.

---

## The Six Phases of Real Agency Client Onboarding

Most onboarding guides compress the process into generic steps. Here's the actual framework — including the phases every guide glosses over.
...
Most of this can be collected through a single intake form — the kind your client fills out while your account manager prepares for the kickoff call. Not a Google Doc. Not an email thread. A form that feeds directly into your process.
...
**LinkedIn**: Campaign Manager → Account Settings → Account Users. Different UI, different terminology, different permission names (VIEW, MODIFY, MANAGER_AD_ACCOUNTS).

**TikTok**: Business Center → Member Management. Requires a QR code authorization for Spark Ads access if applicable.

Five platforms. Five different flows. Five different failure modes. And your account manager has to know all of them cold — plus catch every error the client makes.
...
This verification step catches the errors that Phase 2 inevitably produces. Wrong account. Wrong permissions. Missing assets. Without verification, those errors don't surface until you're building the first campaign — and the client is already losing confidence.
...
This call is effective when the account manager already has platform access and can reference real account data. "I see you're currently running these campaigns on Meta with this budget allocation — let's discuss how we'd approach that differently." beats "Once we get access, we'll take a look."
...
The agencies with the highest client retention don't just launch faster — they communicate better during those first 30 days. Weekly calls, proactive optimization notes, and clear reporting. The client never wonders what's happening.
...
Systematic access collection — a single link that handles all platforms in one client session — takes under an hour. Manual email threads take 1-3 days. That delta determines whether you launch campaigns on Day 3 or Day 7.
...
The intake form isn't just for efficiency — it's for client experience. A professional, branded intake form signals that you have a process. Sending a Google Doc link signals that you're figuring it out as you go.
...
Instead of a kickoff call, then an access email, then an intake form, then strategy — they send a single link. The client clicks through, grants platform access, fills out intake information, and submits it all in one session. No email threads. No PDF instructions. No "I can't find the button" messages.
...
| Phase | Manual Process | Systematic Process | Time Saved |
|-------|---------------|-------------------|-------------|
| Pre-onboarding | 1-2 days | 2-4 hours | 1-1.5 days |
| Platform access | 2-3 days | 30-60 minutes | 2-2.5 days |
| Verification | 30-60 min | 5 min | 25-55 min |
| Strategy & kickoff | 1-2 days | 1-2 days | Minimal |
| Campaign build | 1-2 days | 1-2 days | Minimal |
| **Total to first campaign** | **5-9 days** | **2-4 days** | **3-5 days** |
...
**Number of follow-up emails sent.** A high follow-up count during onboarding correlates with low client satisfaction — it signals disorganization, not diligence. If your account manager sends 8+ emails during onboarding, your process is broken.
...
Client onboarding isn't complicated. It's predictable. The agencies that treat it as a process — not a series of ad-hoc requests — are the ones that scale, retain clients, and stop losing money to email threads about Meta Business Manager access.
